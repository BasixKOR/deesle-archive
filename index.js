...
function mainHandler (err) {
  ...
  server.auth.strategy('jwt-auth', 'jwt', {
    ...
    validateFunc: require(`${__dirname}/utils/authVaildate`),            // validate function defined above
  ...
  server.route([
    ...
    { method: 'POST', path: '/edit/{name}', handler: handlers.wiki.edited, config: { auth: util.auth('try') } }, // 편집 완료 후 핸들러
...
//  { method: 'GET', path: '/search', handler}
  ...
  server.route([
    { method: 'GET', path: '/signin', handler: handlers.auth.signin, config: { auth: util.auth('try') } },
    { method: 'GET', path: '/signout', handler: handlers.auth.signout, config: { auth: util.auth() } },
    { method: 'GET', path: '/register', handler: handlers.auth.register, config: { auth: util.auth('try') } }
  ])
}
